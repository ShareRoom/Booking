$(document).ready(function () {
  ...
  function generateTime(newDate, newMonth, newYear) {
    ...
    timeConfig.map((config) => {
      ...
      const hours = convertTimeToHour(config);
      const customDate = new Date(newYear, Number(newMonth) - 1, newDate, hours, 0, 0, 0);
      console.log('customDate: ', customDate);
      const isSmallerThan12Hours = diffHours(customDate, currentDateTime);
      console.log('isOver12Hours: ', isOver12Hours);

      //Mon to Fri disable 2 to 5pm
      if (
        isSmallerThan12Hours < 12 ||
        (indexDate >= 1 && indexDate <= 5) &&
        (config == timeConfig[5] ||
          config == timeConfig[6] ||
          config == timeConfig[7] ||
          config == timeConfig[8])
      ...
      var timeItems = document.createElement("span");
  ...
  function getBookedItems(date, month, year) {
    ...
    let queryDate = date >= 10 ? `${date}` : `0${date}`
    let qu = "select * where D=date" + `'${year}-${month}-${queryDate}'`;
    console.log('qu: ', qu);
    ...
    fetch(url)
      ...
      .then((rep) => {
        ...
        bookTimeElements.forEach((ele, index) => {
          let eleFlag = 0;
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            if (bookedTimeFormatted == timeConfig[index]) {
              eleFlag += 1;
              if (eleFlag == 2) {
