$(document).ready(function () {
  ...
  function inactiveAllTime() {
    ...
    bookTimeElements.forEach((item) => {
      var itemInput = document.getElementById(
        `time-item-checkbox-${item.innerText}`
      );
      itemInput.checked = false;
      if (itemInput.hasAttribute("name")) {
        itemInput.removeAttribute("name");
  ...
  function generateTime(newDate, newMonth, newYear) {
    ...
    timeConfig.map((config) => {
      ...
      if (
        indexDate >= 1 &&
        indexDate <=5 &&
        (config == timeConfig[10] || config == timeConfig[11] || config == timeConfig[12] || config == timeConfig[13]  || config == timeConfig[14]  || config == timeConfig[15]  || config == timeConfig[16])
      ) {
    ...
    timeArr.map((item) => {
      item.onclick = function () {
        ...
        if (!isDisabled) {
          timeInput.value = item.innerText;
  ...
  function generateDateByMonth(newMonth, newYear) {
    ...
    dates.map((date) => {
      ...
      const currentDate = new Date()
      dateItems.className = 
        (date + 1 < currentDate.getDate() && newMonth == currentDate.getMonth() + 1 && newYear == currentDate.getFullYear())
          ? "date-items-passed"
          : "date-items"
    ...
    dateArr.map((item) => {
      ...
      item.onclick = function () {
        const currentDate = new Date()
        if (
          !(item.firstChild.data < currentDate.getDate() && newMonth == currentDate.getMonth() + 1 && newYear == currentDate.getFullYear())
        ) {
